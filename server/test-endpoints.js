...
async function runTests() {
  console.log('🚀 Starting Node.js Backend Server process...');
  ...
  console.log('🧪 Running automated endpoint tests...\n');
  const results = [];
  ...
  for (const toolEndpoint of pdfTools) {
    try {
      ...
      console.log(`${pass ? '✅' : '❌'} [${res.statusCode}] ${toolEndpoint}`);
    } catch (err) {
      results.push({ name: toolEndpoint, path: toolEndpoint, error: err.message, pass: false });
      console.log(`❌ ${toolEndpoint} - Error: ${err.message}`);
    }
  }
