...
class ScanToPDFService {
  async process(files) {
    ...
    if (!files || files.length === 0) {
      ...
      page.drawText('azPDF Scanner Compiled Page', { x: 50, y: 700, size: 20, font, color: rgb(0.89, 0.14, 0.14) });
    } else {
      for (const file of files) {
        try {
          ...
          const page = pdfDoc.addPage([612, 792]);
          page.drawText(`Scanned Image Stream: ${file.originalname}`, { x: 50, y: 700, size: 14, font, color: rgb(0.2, 0.2, 0.2) });
        }
      }
    }
